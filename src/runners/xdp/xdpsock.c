...
void remove_xdp_program(int optifi) {
  ...
  err = xdp_program__detach(xdp_prog, optifi, XDP_MODE_SKB, 0);
  if (err)
...
#define ETH_HDR_SIZE                                                           \
  (opt_vlan_tag ? sizeof(struct vlan_ethhdr) : sizeof(struct ethhdr))
#define PKTGEN_HDR_SIZE (opt_tstamp ? sizeof(struct pktgen_hdr) : 0)
#define PKT_HDR_SIZE                                                           \
  (ETH_HDR_SIZE + sizeof(struct iphdr) + sizeof(struct udphdr) +               \
   PKTGEN_HDR_SIZE)
#define PKTGEN_HDR_OFFSET                                                      \
  (ETH_HDR_SIZE + sizeof(struct iphdr) + sizeof(struct udphdr))
#define PKTGEN_SIZE_MIN                                                        \
  (PKTGEN_HDR_OFFSET + sizeof(struct pktgen_hdr) + ETH_FCS_SIZE)

#define PKT_SIZE (opt_pkt_size - ETH_FCS_SIZE)
#define IP_PKT_SIZE (PKT_SIZE - ETH_HDR_SIZE)
#define UDP_PKT_SIZE (IP_PKT_SIZE - sizeof(struct iphdr))
#define UDP_PKT_DATA_SIZE                                                      \
  (UDP_PKT_SIZE - (sizeof(struct udphdr) + PKTGEN_HDR_SIZE))
...
struct xsk_socket_info *xsk_configure_socket(struct xsk_umem_info *umem,
...
void l2fwd(struct xsk_socket_info *xsk, int tunfd,
  ...
  for (i = 0; i < rcvd; i++) {
    ...
    if (write(tunfd, pkt, len) < 0) {
      printf("There was an error writing to the TAP interface: %d, %s.\n",
             errno, strerror(errno));
    }

    printf("forwarding down the tap ...");
...
int tun_alloc_aper(const char *dev, const char *dev_to_ape) {
  ...
  if ((fd = open("/dev/net/tun", O_RDWR, O_NONBLOCK)) < 0) {
    printf("There was an error creating the tun/tap interface: %s\n",
           strerror(errno));
    return -1;
  }
  ...
  if (nl < 0) {
    printf("There was an error getting the netlink socket: %s\n",
           strerror(errno));
    return -1;
  }

  if (netlink_link_up(nl, "tapst0") < 0) {
    printf("There was an error closing the netlink socket: %s\n",
           strerror(errno));
    return -1;
  }

  struct sockaddr aperaddr;
  if (netlink_get_addr(nl, dev_to_ape, &aperaddr) < 0) {
    printf("There was an error getting the adddress of the link to ape: %s\n",
           strerror(errno));
    return -1;
  }
  ...
  if (netlink_set_addr(fd, "tapst0", &aperaddr) < 0) {
    printf("There was an error setting the address of the tap: %s\n",
           strerror(errno));
    return -1;
  }
