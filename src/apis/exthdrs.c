...
bool add_extension(extensions_p *extensions, size_t *index) {
    ...
    extension_p **new_extension_list = (extension_p**)realloc(extensions->extensions_values, (current_size + 1) * sizeof(extension_p*));
...
bool remove_extension(extensions_p *extensions, size_t index) {
    ...
    if (extensions->extensions_count == 1) {
        ...
        extensions->extensions_values[index] = extensions->extensions_values[extensions->extensions_count -1];

        extension_p **new_extension_list = (extension_p**)realloc(extensions->extensions_values, (extensions->extensions_count - 1) * sizeof(extension_p*));
...
bool coalesce_extensions(extensions_p *extensions, uint8_t type) {
    ...
    while (find_next_extension(*extensions, &next_index, type)) {
        ...
        uint8_t new_size = extensions->extensions_values[first_index]->len + extensions->extensions_values[next_index]->len;
        uint8_t *new_data = (uint8_t*)calloc(new_size, sizeof(uint8_t));
        memcpy(new_data, extensions->extensions_values[first_index]->data, extensions->extensions_values[first_index]->len);
        memcpy(new_data + extensions->extensions_values[first_index]->len, extensions->extensions_values[next_index]->data, extensions->extensions_values[next_index]->len);
